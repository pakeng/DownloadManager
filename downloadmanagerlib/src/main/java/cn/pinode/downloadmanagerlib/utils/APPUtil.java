...
public class APPUtil {
    ...
    private static void installWithFileProvider(Context context, File file, DownloadTask task) {
        ...
        Uri apkUri = FileProviderHelper.getUriForFile(context,  file);
    ...
    public static void installApkWithTask(Context context, DownloadTask task) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            File apkFile = new File(task.getDownloadDestination().getPath()+task.getFileName());
            installWithFileProvider(context, apkFile, task);
        } else {
            File apkFile = new File(task.getDownloadDestination().getPath()+task.getFileName());
            installApk(context, Uri.parse(task.getDownloadDestination()+task.getFileName()), task);
        }
    }
